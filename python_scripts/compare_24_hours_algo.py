...
CSV_FILE = "../outputs/algorithm_comparison_24h.csv"

# Simulation constants
SIM_SECONDS_PER_HOUR = 3600
TOTAL_SIM_TIME = 86400  # 24 hours
...
def run_algorithm(algorithm, algorithm_name, graph, pos):
    ...
    while traci.simulation.getTime() < TOTAL_SIM_TIME:
        ...
        # Run ACO updates
        if algorithm == a_star_traffic_pheromone and current_time % ACO_UPDATE_INTERVAL == 0 and current_time % SIM_SECONDS_PER_HOUR != 0 and hour < 24:
        ...
        if current_time >= hour * SIM_SECONDS_PER_HOUR and hour < 24:
...
def main():
    try:
        ...
        for algorithm, algorithm_name in algorithms:
            print(f"\nStarting 24-hour simulation for {algorithm_name}")
